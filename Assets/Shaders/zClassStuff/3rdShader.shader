Shader "Unlit/My2ndUnlitShader"
{
    Properties
    {
        ...
        _RimColor ("Rim Color", Color) = (1,1,1,1) //added
        _RimAmount ("Rim Amount", Range(0, 1)) = 0.716 //added
    }
    SubShader
    {
        // memes are dreams
        ...
        Pass
        {
            ...
            float4 _RimColor; //added
            float _RimAmount; //added
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float specAdjustment = smoothstep(0.0, 1, specularIntensity);
                float specular = specAdjustment * _SpecularColor; //added

                float4 rimDot = dot(viewDir, normal); //added
                float rimIntensity = smoothstep(_RimAmount - 0.01, _RimAmount + 0.01, rimDot); //added
                float4 rim = rimIntensity * _RimColor; //added
                // sample the texture
                fixed4 col = tex2D(_MainTex, i.uv);
                // apply fog
                UNITY_APPLY_FOG(i.fogCoord, col);
                return col * _Color * (light + _AmbientColor + specularIntensity + specular - rim); //added _Color
            }
            ENDCG
        }
